    ...
    if (user) {
      authenticatedNav.append(
        Aim('li').class('nav-item').append(
          ...
          .text('Contacts').on('click', e => getContacts())
        )
      )
    }
  },
  accountNav(user) {
    ...
    if (user) {
      accountNav.class('nav-item dropdown').append(
        ...
        .append(
          ...
          Aim('div').class('dropdown-menu dropdown-menu-right').append(
            ...
            Aim('button').class('dropdown-item').text('Sign out')
            .on('click', e => signOut()),
            Aim('div').class('dropdown-divider'),
            Aim('button').class('dropdown-item').text('Upload config')
      ...
      accountNav.class('nav-item').append(
        ...
        .text('Sign in').on('click', e => signIn())
      );
    }
  },
  home(user) {
    ...
    if (user) {
      ...
      mainContainer.append(
        Aim('button').class('btn btn-primary btn-large')
        .text('Click here to sign in').on('click', e => signIn())
      )
    }
  },
  error(error) {
  ...
  Aim('main-container').text('').append(
    ...
    Aim('table').class('table').append(
      ...
      Aim('tbody').append(
        items.map(
          item => Aim('tr').append(
            Aim('td').text(item.name),
            Aim('td').text(item.url),
            Aim('td').append(
              Views.item
              ? Aim('a').text('View').href('#')
              .on('click', e => getContact(item))
              : null,
            ),
          )
        )
      ...
      Aim('tbody').append(
        Object.entries(item.properties).map(([propertyName,property]) => {
          return Aim('tr').append(
            Aim('td').text(propertyName),
            Aim('td').append(
              Aim('input').name(propertyName).value(item[propertyName]),
            ),
          )
        }),
      ),
